...
subsets_BPS <- read_csv(file = "BPS_subsets_filtered.csv")
...
G_power$Treatment <- factor(G_power$Treatment, ordered = T, levels = c("Morphine", "Pure THC"))
...
ggplot(subsets_BPS, aes(x = Subset, y = Latency)) +
  geom_point(position = position_jitter(width = 0.20), 
             alpha = 0.25, size = 3, pch = 19, colour = "grey30") +
